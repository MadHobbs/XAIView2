...
from xView2_first_place.zoo.models import Res34_Unet_Double
...
for f in tqdm(sorted(listdir(test_dir))):
    if '_pre_' in f:
        ...
        inp.requires_grad_()
        msk = model(inp)
        msk = torch.sigmoid(msk) # convert to "probability" scores
        pred = torch.mean(msk, 0) # average over 4 layers
        
        labels = ['no-building', 'no-damage', 'minor-damage', 
                  'major-damage', 'destroyed']
        for i,lab in enumerate(labels):
            # whether_above_thresh = pred[i] > PRED_THRESHOLD
            # indices_above_thresh = whether_above_thresh.nonzero()
            # value_of_interest = torch.sum(
            #                 torch.where(pred[i] > PRED_THRESHOLD, 
            #                             pred[i],                       # if
            #                             torch.zeros(pred[i].shape[0],  # else 
            #                                         pred[i].shape[1]))
            #                         )/len(indices_above_thresh)
            value_of_interest = torch.mean(torch.topk(pred[i].flatten(),TOP_K).values)
            value_of_interest.backward(retain_graph=True) # calculate gradient
            ...
            fname = save_dir + fn.split("/")[-1].split(".")[0]+"_dmg_"+lab+"_saliency_"+str(PRED_THRESHOLD)+".png"
